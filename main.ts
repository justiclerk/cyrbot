...
console.log(
  `starting cyrbot ${packageInfo.version} (on Deno ${Deno.version.deno})`,
);

const AIRTABLE_BASE_ID = Deno.env.get("AIRTABLE_BASE_ID")!;
const AIRTABLE_TABLE_ID = encodeURIComponent(
  Deno.env.get("AIRTABLE_TABLE_ID")!,
);
const AIRTABLE_API_KEY = Deno.env.get("AIRTABLE_API_KEY")!;
const AIRTABLE_ENDPOINT_URL = Deno.env.get("AIRTABLE_ENDPOINT_URL") ||
...
const listRecordsBaseUrl =
  `${AIRTABLE_ENDPOINT_URL}/v0/${AIRTABLE_BASE_ID}/${AIRTABLE_TABLE_ID}?filterByFormula=bluesky_did`;
...
async function refreshRecords() {
  console.log(
    `${ms(ms(REFRESH_INTERVAL), { long: true })} passed, refreshing records`,
  );
  await updateAirtableRecords();
